...
class Actions():
    ...
    def __init__(self):
        ...
        self.actions = np.vstack(
            [self.actions, np.mgrid[0.5:0.6:0.5, -np.pi / 6:np.pi / 6 + 0.01:np.pi / 6].reshape(2, -1).T])
        self.actions = np.vstack(
            [self.actions, np.mgrid[0.0:0.1:0.5, -np.pi / 6:np.pi / 6 + 0.01:np.pi / 6].reshape(2, -1).T])
...
class NetworkVPCore(object):
    ...
    def _create_graph_outputs(self):
        # FCN
        self.fc1 = tf.layers.dense(inputs=self.final_flat, units=256, use_bias=True, activation=tf.nn.relu,
        ...
        self.logits_v = tf.squeeze(
            tf.layers.dense(inputs=self.fc1, units=1, use_bias=True, activation=None, name='logits_v'), axis=[1])
    ...
    def predict_p_from_lstm_output(self, host_agent_vec, lstm_output):
        return self.sess.run(nn.softmax_p,
                             feed_dict={self.host_agent_vec: host_agent_vec, self.layer1_input: layer1_input})
...
class NetworkVP_rnn(NetworkVPCore):
    ...
    def _create_graph(self):
        ...
        if Config.MULTI_AGENT_ARCH == 'RNN':
            ...
            self.other_agent_seq = tf.reshape(self.other_agent_vec,
                                              [-1, max_length, Config.OTHER_AGENT_FULL_OBSERVATION_LENGTH])
            self.rnn_outputs, self.rnn_state = tf.nn.dynamic_rnn(tf.contrib.rnn.LSTMCell(num_hidden),
                                                                 self.other_agent_seq, dtype=tf.float32,
            ...
            self.layer1 = tf.layers.dense(inputs=self.layer1_input, units=256, activation=tf.nn.relu,
